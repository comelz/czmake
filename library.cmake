...
FUNCTION(CREATE_TARGET NAME)
    ...
    foreach(TGT ${TGTS})
        ...
        if(CMAKE_COMPILER_IS_GNUCXX)
            get_target_property(PROPS ${TGT} COMPILE_OPTIONS)
            if(NOT PROPS)
                set(PROPS)
            endif()
            list(APPEND PROPS -Wall -Wextra -pedantic -pthread)
            if(CMAKE_BUILD_TYPE STREQUAL "Debug")
                target_compile_definitions(${TGT} PRIVATE _GLIBCXX_DEBUG)
                string(APPEND PROPS -ftrapv)
            endif()
            set_target_properties(${TGT} PROPERTIES COMPILE_OPTIONS "${PROPS}")
            get_target_property(PROPS ${TGT} LINK_FLAGS)
            if(NOT PROPS)
                set(PROPS)
            endif()
            string(APPEND PROPS " -pthread ")
            set_target_properties(${TGT} PROPERTIES LINK_FLAGS "${PROPS}")
        endif()
    endforeach()
ENDFUNCTION()
